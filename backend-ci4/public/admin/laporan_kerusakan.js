...
let modalPelaksanaInstance;
let modalValidasiInstance;
let statusModalEdit;

let currentButton = null;
let currentValidasiButton = null;
...
window.addEventListener("DOMContentLoaded", () => {
  // A. Init Modal Unit
  const modalUnitEl = document.getElementById("modal_unit");
  if (modalUnitEl) modalUnitInstance = new bootstrap.Modal(modalUnitEl);

  // B. Init Modal Tambah
  const elModalTambah = document.getElementById("ModalTambahLaporan");
  if (elModalTambah) {
    modalTambahInstance = new bootstrap.Modal(elModalTambah);
    const formTambah = document.getElementById("formTambahLaporan");
    if (formTambah)
      formTambah.addEventListener("submit", window.handleSubmitTambah);
  }

  // C. Init Modal Edit
  const elModalEdit = document.getElementById("ModalEditLaporan");
  if (elModalEdit) {
    statusModalEdit = new bootstrap.Modal(elModalEdit);
    const formEdit = document.getElementById("formEditLaporan");
    if (formEdit) formEdit.addEventListener("submit", window.handleSubmitEdit);
  }

  // D. Init Modal Pelaksana (Jadwal)
  const elModalPelaksana = document.getElementById("modalPelaksana");
  if (elModalPelaksana) {
    modalPelaksanaInstance = new bootstrap.Modal(elModalPelaksana);
    const formPelaksana = document.getElementById("formPelaksana");
    if (formPelaksana)
      formPelaksana.addEventListener("submit", window.handleSubmitPelaksana);
  }

  // E. Init Modal Validasi
  const elModalValidasi = document.getElementById("validasiModal");
  if (elModalValidasi) {
    modalValidasiInstance = new bootstrap.Modal(elModalValidasi);
    const btnConfirm = document.getElementById("btnConfirmValidasi");
    if (btnConfirm)
      btnConfirm.addEventListener("click", window.confirmValidasi);
  }

  // F. Init Pagination & Search
  // initPagination();

  // G. Init Sidebar
  setupSidebar();

  // H. Init Logika Form Tambah (Autofill)
  initFormLogicTambah();

  // I. Muat Ulang UI Tombol Pelaksana
  // loadSavedPelaksanaData();
...
// =======================================================
// 3. LOGIKA FORM TAMBAH (AUTOFILL & CASCADING)
// =======================================================
function initFormLogicTambah() {
  ...
  if (selectAlat) {
    selectAlat.addEventListener("change", function () {
      ...
      const noInventaris = selectedOption.getAttribute("data-inventaris");

      if (inputInventaris)
        inputInventaris.value = noInventaris ? noInventaris : "";
      if (inputPelaksana) {
        inputPelaksana.value = pelaksanaMap[namaAlat]
          ? pelaksanaMap[namaAlat]
          : "";
      }
    });
  }
  ...
  if (selectJurusan && selectLokasi) {
    selectJurusan.addEventListener("change", function () {
      ...
      if (selectedUnit && dataMasterLokasi[selectedUnit]) {
        selectLokasi.disabled = false;
        dataMasterLokasi[selectedUnit].forEach(function (ruangan) {
          const option = document.createElement("option");
          option.value = ruangan;
          option.textContent = ruangan;
          selectLokasi.appendChild(option);
        });
      } else if (["Lainnya", "KPA", "Pos Satpam"].includes(selectedUnit)) {
        selectLokasi.disabled = false;
        const option = document.createElement("option");
        option.value = "Lokasi Umum " + selectedUnit;
        option.textContent = "Lokasi Umum " + selectedUnit;
        selectLokasi.appendChild(option);
      } else {
...
// =======================================================
// 4. FUNGSI CRUD & MODAL (TAMBAH, EDIT, VALIDASI, PELAKSANA)
// =======================================================
const BASE_URL_TAMBAH = BASE_URL + "admin/tambah_data";
const BASE_URL_GET_NOMOR = BASE_URL + "admin/get_nomor_otomatis";
...
async function handleSimpanData(e) {
  ...
  try {
    ...
    if (result.status === "success") {
      if (modalTambahInstance) modalTambahInstance.hide();
      setTimeout(() => {
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
      ...
      Swal.fire({ 
        ...
        title: "Gagal", 
        text: errorMsg,
        footer: result.debug ? '<small>' + result.debug + '</small>' : ''
      });
    }
  } catch (error) {
...
function hapusLaporan(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const url = BASE_URL + "admin/hapus_laporan";
      fetch(url, {
        method: "POST",
        body: formData,
        headers: { "X-Requested-With": "XMLHttpRequest" },
...
async function openModalEdit(nomorLaporan) {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}admin/get_data_by_nomor/${nomorLaporan}`,
      { headers: { "X-Requested-With": "XMLHttpRequest" } }
    );
...
async function handleSubmitEdit(e) {
  ...
  const formData = new FormData(this);
  try {
    const response = await fetch(`${BASE_URL}admin/update_data`, {
      method: "POST",
      body: formData,
      headers: { "X-Requested-With": "XMLHttpRequest" },
    });
...
window.openValidasiModal = function (button, nomorLaporan) {
  currentValidasiButton = button;
  currentLaporanId = nomorLaporan;
  modalValidasiInstance.show();
};

window.confirmValidasi = async function () {
  if (!currentLaporanId) return;
  const btn = document.getElementById("btnConfirmValidasi");
  btn.disabled = true;
  ...
  try {
    ...
    if (result.status === "success") {
      if (currentValidasiButton) {
        currentValidasiButton.classList.remove("btn-danger");
        currentValidasiButton.classList.add("btn-success");
        currentValidasiButton.innerHTML = '<i class="fas fa-check"></i>';
        currentValidasiButton.disabled = true;
      }
      modalValidasiInstance.hide();
    } else {
      Swal.fire({
        icon: "error",
        title: "Gagal",
    ...
    btn.disabled = false;
    btn.innerHTML = "Ya, Validasi";
  }
};

// =======================================================
// FUNGSI EXPAND ROW (LAYOUT TABEL/KOTAK)
// =======================================================
window.toggleDetailRow = async function (btn, nomorLaporan) {
  ...
  try {
    ...
    if (result.status === "success") {
      ...
      const childRowHtml = `
                <tr class="child-row bg-light fade-in">
                    <td colspan="8" class="p-4">
                        <div class="card shadow-sm border-0">
                            <div class="card-header bg-secondary text-white py-2">
                                <h6 class="mb-0 small fw-bold"><i class="fas fa-list me-2"></i> Detail Tambahan: ${
                                  d.nomor_laporan
                                }</h6>
                            </div>
                            <div class="card-body p-0">
                                <table class="table table-bordered mb-0 text-center">
                                    <thead class="table-light small text-muted text-uppercase">
                                        <tr>
                                            ...
                                            <th width="25%">Tanggal Perbaikan</th>
                                            <th width="25%">Validasi Kepala</th>
                                            <th width="25%">Status Perbaikan</th>
                                        </tr>
                                    </thead>
                                    <tbody class="small fw-bold text-dark bg-white">
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                ${
                                                  d.validasi_kepala == "1" ||
                                                  d.validasi_kepala == true
                                                    ? '<span class="badge bg-success"><i class="fas fa-check me-1"></i> Valid</span>'
                                                    : '<span class="badge bg-danger"><i class="fas fa-times me-1"></i> Belum</span>'
                                                }
                                            </td>
                                            <td class="align-middle">
                                ...
                                <table class="table table-bordered mb-0 border-top-0">
                                    <tbody class="small">
                                        <tr>
                                            <th width="20%" class="table-light align-middle text-center text-uppercase text-muted">Kerusakan / Keluhan</th>
                                            <td class="bg-white p-3 align-middle text-start">${
                                              d.kerusakan_keluhan || "-"
                                            }</td>
                                        </tr>
                                        <tr>
                                            <th class="table-light align-middle text-center text-uppercase text-muted">Bukti Foto</th>
                                            <td class="bg-white p-3 align-middle text-start">
                                                ${
                                                  d.foto && d.foto.length > 0
                                                    ? d.foto
                                                        .map(
                                                          (f) =>
                                                            `<a href="${BASE_URL}uploads/${f}" target="_blank" class="me-2" title="Klik untuk memperbesar">
                                                          <img src="${BASE_URL}uploads/${f}" class="img-thumbnail rounded" style="height: 80px; width: 80px; object-fit: cover;">
                                                       </a>`
                                                        )
                                                        .join("")
            ...
            `;
      tr.insertAdjacentHTML("afterend", childRowHtml);
...
// ======================================================
window.openStatistikModal = function () {
  let total = 0;
  let countMenunggu = 0;
  let countSelesai = 0;
  ...
  const statsJurusan = {};
  const statsAlat = {};
  const statsSeverity = { Ringan: 0, Sedang: 0, Berat: 0 };
  const statsPerbaikan = {};

  const rows = document.querySelectorAll(
    "#tableLaporan tbody tr:not(.child-row):not(.empty-row)"
  );

  rows.forEach((tr) => {
    const rawJson = tr.getAttribute("data-json");
    if (!rawJson) return;

    const d = JSON.parse(rawJson);
    total++;

    const sev = d.status_kerusakan || "";
    if (sev.includes("Ringan")) statsSeverity["Ringan"]++;
    else if (sev.includes("Sedang")) statsSeverity["Sedang"]++;
    else if (sev.includes("Berat")) statsSeverity["Berat"]++;

    const jur = d.jurusan_unit || d.unit || "Lainnya";
    statsJurusan[jur] = (statsJurusan[jur] || 0) + 1;

    const alat = d.nama_alat || "Tanpa Nama";
    statsAlat[alat] = (statsAlat[alat] || 0) + 1;

    let rawStatus = d.status_perbaikan || "";
    let statusKategori = "Menunggu";

    if (rawStatus.includes("Selesai")) {
      statusKategori = "Selesai";
    } else if (rawStatus.includes("Pending")) {
      statusKategori = "Pending";
    } else if (
      rawStatus.includes("Proses") ||
      rawStatus.includes("Dikerjakan")
    ) {
      statusKategori = "Dikerjakan/Diperbaiki";
    } else if (rawStatus.includes("Rusak") || rawStatus.includes("Gagal")) {
      statusKategori = "Masih Rusak";
    }

    statsPerbaikan[statusKategori] = (statsPerbaikan[statusKategori] || 0) + 1;

    if (statusKategori === "Menunggu" || statusKategori === "Pending")
      countMenunggu++;
    if (statusKategori === "Selesai") countSelesai++;
  });

  document.getElementById("statTotal").innerText = total;
  document.getElementById("statMenunggu").innerText = countMenunggu;
  document.getElementById("statSelesai").innerText = countSelesai;
  document.getElementById("statJenisAlat").innerText =
    Object.keys(statsAlat).length;

  const sortedJur = Object.entries(statsJurusan).sort((a, b) => b[1] - a[1]);
  renderWebChart("webChartJurusan", "bar", {
    labels: sortedJur.map((i) => i[0]),
    data: sortedJur.map((i) => i[1]),
    colors: "#0d6efd",
    indexAxis: "y",
  });

  renderWebChart("webChartSeverity", "doughnut", {
    labels: ["Ringan", "Sedang", "Berat", "Rusak"],
    data: [
      statsSeverity["Ringan"],
      statsSeverity["Sedang"],
      statsSeverity["Berat"],
      statsSeverity["Rusak"] || 0,
    ],
    colors: ["#198754", "#ffc107", "#dc3545", "#212529"],
  });

  const sortedAlat = Object.entries(statsAlat).sort((a, b) => b[1] - a[1]);
  renderWebChart("webChartAlat", "bar", {
  ...
  const targetStatus = [
    { label: "Menunggu", color: "#6c757d" },
    { label: "Pending", color: "#ffc107" },
    { label: "Dikerjakan/Diperbaiki", color: "#0dcaf0" },
    { label: "Selesai", color: "#198754" },
    { label: "Masih Rusak", color: "#dc3545" },
  ];

  let finalLabels = [];
  let finalData = [];
  let finalColors = [];
  targetStatus.forEach((item) => {
    finalLabels.push(item.label);
    finalColors.push(item.color);
    finalData.push(statsPerbaikan[item.label] || 0);
  });

  renderWebChart("webChartStatus", "pie", {
    labels: finalLabels,
    data: finalData,
    colors: finalColors,
  });

  const modalEl = document.getElementById("modalStatistik");
  document.body.appendChild(modalEl);
  new bootstrap.Modal(modalEl).show();
};

function renderWebChart(canvasId, type, config) {
...
function updateDashboardStats() {
  ...
  fetch(`${BASE_URL}admin/get_statistik_json?${params.toString()}`)
    .then((response) => {
...
$(document).ready(function () {
  var table = $("#tableLaporan").DataTable({
    ...
    ajax: {
      url: BASE_URL + "admin/get_laporan_historis",
      type: "GET",
      data: function (data) {
